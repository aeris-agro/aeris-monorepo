...
def download_latest_geojson(prefix: str) -> dict:
    ...
    if not files:
        raise FileNotFoundError(
            f"No GeoJSON files found in gs://{GCS_BUCKET}/{prefix}/"
        )
    latest = sorted(files)[-1]
    print(f"Downloading: {latest}")
    content = subprocess.run(
        ["gsutil", "cat", latest], capture_output=True, text=True, check=True
    ).stdout
...
def get_or_create_district(name: str) -> str:
    ...
    ins = (
        db_shared("districts")
        .insert(
            {"name": name, "code": name.lower()[:3], "geometry": "MULTIPOLYGON EMPTY"}
        )
        .execute()
    )
...
def get_or_create_sub_county(name: str, district_id: str) -> str:
    ...
    ins = (
        ...
        .insert(
            {"name": name, "district_id": district_id, "geometry": "MULTIPOLYGON EMPTY"}
        )
        .execute()
    )
    return ins.data[0]["id"]
...
def ingest_ndvi():
    ...
    for feature in geojson.get("features", []):
        ...
        record = {
            ...
            "red_edge_mean": (
                round(props["RedEdge_mean"], 4) if props.get("RedEdge_mean") else None
            ),
...
def ingest_rainfall():
    ...
    for feature in geojson.get("features", []):
        ...
        print(
            f"  ✓ {sc_name} ({district_name}): {rainfall_mm:.1f}mm | anomaly={anomaly_pct:.1f}%{flag}"
        )
        inserted += 1
