...
def get_active_alerts():
    ...
    """
    # 1. Fetch alerts (no embedded join — sub_counties lives in shared schema)
    alerts = (
    ...
    # 2. Fetch sub-county names from shared schema and build a lookup
    ...
    for row in alerts.data:
        sc_name = sc_lookup.get(row.get("sub_county_id"), "Unknown")
        results.append(
            AlertResponse(
                id=str(row["id"]),
                sub_county=sc_name,
