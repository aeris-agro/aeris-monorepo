...
def get_rainfall_history(
    sub_county: Optional[str] = Query(None, description="Filter to one sub-county (Lira/Alebtong/Dokolo)"),
    days:       int           = Query(30, ge=1, le=90, description="Number of days back from latest CHIRPS observation"),
):
    """
    ...
    latest_res = (aeryion("rainfall_observations")
    ...
    rows = (aeryion("rainfall_observations")
    ...
    series_by_sc: dict[str, List[DailyRainfallPoint]] = {nm: [] for nm in sc_map.values()}
    for row in rows.data or []:
        sc_name = sc_map.get(row["sub_county_id"])
        if sc_name is None:
            continue
        series_by_sc[sc_name].append(DailyRainfallPoint(
