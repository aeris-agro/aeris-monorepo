...
def expected_ndvi(crop: str, days_planted: int) -> tuple[float, float]:
    ...
    if pct < 0.15:    return (0.20, 0.40)
    elif pct < 0.50:  return (0.45, 0.75)
    elif pct < 0.80:  return (0.55, 0.85)
    else:             return (0.30, 0.60)
...
def build_advisories(plot: dict, sc_name: str) -> tuple[list[AdvisoryItem], dict]:
    ...
    if ndvi_mean is not None and days_planted is not None and days_planted > 0:
        ...
        if ndvi_mean < lo:
            advisories.append(AdvisoryItem(
                category="vegetation", severity="warning",
                title=f"Crop stress detected ({crop})",
                body=f"Your {crop} field shows NDVI {ndvi_mean:.2f}, below the healthy range of {lo:.2f}–{hi:.2f} for day {days_planted}. May indicate water stress, pest damage, or nutrient deficiency.",
                action="Inspect field within 48hrs. Check for pests on leaves and verify soil moisture.",
            ))
        elif ndvi_mean > hi:
            advisories.append(AdvisoryItem(
                category="vegetation", severity="info",
    ...
    if forecast_total is not None:
        if forecast_total > 50:
            advisories.append(AdvisoryItem(
                category="rainfall", severity="warning",
            ...
            advisories.append(AdvisoryItem(
                category="rainfall", severity="info",
    ...
    if soil_ph is not None:
        if soil_ph < 5.5:
            advisories.append(AdvisoryItem(
                category="soil", severity="warning",
                title="Acidic soil",
    ...
    if days_planted is not None and crop in CROP_CYCLE_DAYS:
        ...
        if 0 < days_remaining <= 14:
            advisories.append(AdvisoryItem(
                category="harvest", severity="info",
