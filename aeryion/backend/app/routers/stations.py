...
def get_stations():
    ...
    """
    res = aeryion("iot_stations").select("*").order("station_code").execute()
    if not res.data:
        return []
    ...
    return [
        StationResponse(
            ...
            last_reading_at=(
                str(row["last_reading_at"]) if row.get("last_reading_at") else None
            ),
...
def get_station_readings(
    station_id: str,
    hours: int = Query(
        24, ge=1, le=168, description="Hours of history to return (max 168 = 7 days)"
    ),
):
    """Last N hours of hourly readings from one station."""
    station = (
        aeryion("iot_stations")
        .select("id, name")
        .eq("id", station_id)
        .limit(1)
        .execute()
    ...
    readings = (
        aeryion("station_readings_hourly")
        .select("*")
